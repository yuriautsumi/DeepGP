...
for i in range(0,1): #0 to 9
    ...
    base_weights, z, base_model = call_base_model_drop_reg(x_s, y_s, xtest_all, ytest_all, drop, reg, deep_layer_dims = 128)
...
#    for b in range(100):
        ...
        deep_layer_dgp1 = Dense(128, activation = 'relu', weights = dnn_weights, trainable = True, kernel_regularizer=regularizers.l2(reg), activity_regularizer=regularizers.l1(reg))(inputs_dgp1)
        dropout1 = Dropout(drop)(deep_layer_dgp1)
        outputs_dgp1 = gp_layer1(dropout1)
        ...
        deep_layer_dgp2 = Dense(128, activation = 'relu', weights = dnn_weights, trainable = False, kernel_regularizer=regularizers.l2(reg), activity_regularizer=regularizers.l1(reg))(inputs_dgp2)
        dropout2 = Dropout(drop)(deep_layer_dgp2)
        outputs_dgp2 = gp_layer2(dropout2)
        ...
        for ID in tr_ind_source: 
            ...
            m_s_patient, s_s_patient, y_a_patient  = save_to_mat(MAT_LOOP_TRAINING_FOLDER_DIR, ID, x_s, y_s, x_dict_s, y_dict_s, y_dict_s, deep_model2, sgp_train_m, sgp_train_s, tr_inds, optimized_params=None, gp_layer=gp_layer2, kgp_model=True, tst_pat=False)
        
        for ID in tst_ind: 
            
            print('----- TEST PATIENT: %s -----'%(ID))
            
            m_s_patient, s_s_patient, y_a_patient  = save_to_mat(MAT_LOOP_TESTING_FOLDER_DIR, ID, x_s, y_s, x_a, y_a, g_t, deep_model2, sgp_test_m, sgp_test_s, tst_inds, optimized_params=None, gp_layer=gp_layer2, kgp_model=True, tst_pat=True)
    ...
    for ID in tst_ind: 
        ...
        m_s_patient, s_s_patient, y_a_patient = save_to_mat_final(MAT_FINAL_FOLDER_DIR, ID, x_s, y_s, x_a, y_a, g_t, deep_model2, gp_layer2, sgp_test_m, sgp_test_s, tst_inds, base_weights)
